...
def main():
    ...
    # Set parameters
    n_minimax = 1                     # Number of minimax points
    R_minimax = int(1000)                 # Range of the minimax approximation
    n_x       = 8000                   # total number of points on the x-axis for optimization
    eps_diff  = 10**(-10)

    xdata = 10**(np.logspace(0,np.log10(np.log10(R_minimax)+1),n_x,dtype=np.float64))/10
#    print("xdata", xdata)
    ydata = np.zeros(n_x,dtype=np.float64)

    alphas_betas_init = np.loadtxt("../alpha_beta_of_N_"+str(n_minimax),dtype=np.float64)
...
def eta_for_alphas_betas_E_update(x, *params):
    ...
    size_x = np.size(x)
    E = np.empty(size_x, dtype=np.float64)
