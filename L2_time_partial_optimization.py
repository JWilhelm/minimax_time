...
def main():
    ...
    alphas_betas_init = np.append(alphas_betas_init[0:n_opt] , alphas_betas_init[n_minimax:n_minimax+n_opt] )

    print("alphas_betas_init", alphas_betas_init)
    print("np.shape(alphas_betas_init)", np.shape(alphas_betas_init))

    alphas_betas_L2_opt, alphas_betas_conv = curve_fit(eta, xdata, ydata, p0=alphas_betas_init, ftol=1e-10, gtol=1e-10)

    sort_indices = np.argsort(alphas_betas_L2_opt[0:n_opt])
    np.savetxt("alpha_beta_of_N_"+str(n_minimax)+"_L2", np.append(alphas_betas_L2_opt[sort_indices],alphas_betas_L2_opt[sort_indices+n_opt]) )

#    fig1, (axis1) = pl.subplots(1,1)
#    axis1.set_xlim((0.8,R_minimax))
#    axis1.semilogx(xdata,eta_plotting(xdata,np.append(alphas_betas_L2_opt))
